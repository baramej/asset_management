...
the Odoo company records:
  ...
  * the user and employee are created under that entry's Default Company,
    e.g. SBA, because Auto Arab has no res.company of its own,
...
The employee is matched on employee_code inside the default company,
accepting both "78723" and "AA78723". If Odoo does not have them the legacy
HR service is asked; a code it does not know is rejected. Only when the
service is not configured or unreachable is a thin employee created, flagged
as self-registered for HR to review.
"""
import logging
...
class AssetPortalCompany(models.Model):
    ...
    company_id = fields.Many2one(
        ...
        help="Odoo company the portal user and the employee are created "
        "under, e.g. SBA.",
    )
...
class HrEmployee(models.Model):
    ...
    portal_company_id = fields.Many2one(
        ...
        "when it is held under another Odoo company (e.g. SBA).",
    )
    ...
    # ------------------------------------------------------------------ #
    # Legacy HR sync: query with the actual company's code
    # ------------------------------------------------------------------ #
    def _legacy_company_code(self):
        """Auto Arab employees held under SBA are known to the legacy HR
        system by Auto Arab's code, not SBA's."""
        self.ensure_one()
    ...
    def _asset_portal_belongs_to(self, portal_company):
        ...
        Several entities share one Odoo company (SBA), so the same number can
        exist under more than one of them. Anything explicitly tied to another
        entity, by portal_company_id or by the legacy companyCode mirrored by
        the daily sync, is ruled out.
        """
        self.ensure_one()
        if self.portal_company_id:
            return self.portal_company_id == portal_company
        legacy_code = _clean_code(self["emp_company_code"]) if "emp_company_code" in self._fields else ""
        if legacy_code:
            return legacy_code == _clean_code(portal_company.code)
        return True
        ...
        if employee:
            ...
            if legacy_values:
                ...
                note = _("Created from portal sign-up (%(company)s, code %(code)s): the "
                         ...
                         company=portal_company.name, code=resolved["employee_code"])
            employee = Employee.with_company(company).create(vals)
            employee.message_post(body=note)
        _logger.info(
            "Portal user %s linked to employee %s (%s) of %s, held under %s",
            user.login, employee.id, employee.employee_code, portal_company.name, company.name,
        )
        return employee
